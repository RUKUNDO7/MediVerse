...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            Role userRole = roleRepository.findByName(Role.RoleType.ROLE_PATIENT)
            ...
            strRoles.forEach(role -> {
                switch (role.toLowerCase()) {
                    case "admin":
                        ...
                        break;
                    case "doctor":
                        Role modRole = roleRepository.findByName(Role.RoleType.ROLE_DOCTOR)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(modRole);
                        break;
                    case "nurse":
                        Role nurseRole = roleRepository.findByName(Role.RoleType.ROLE_NURSE)
                                .orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(nurseRole);
                        break;
                    default:
